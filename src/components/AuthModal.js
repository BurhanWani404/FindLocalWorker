...
const AuthModal = ({ onClose, flowType = "worker" }) => {
  ...
  const isLoading = loading || isSigningIn || isSigningUp || isProcessingReset;
  ...
  const validateForm = () => {
    ...
    if (view === "auth" && isLogin && !formData.password) {
      newErrors.password = "Password is required";
    } else if (view === "auth" && isLogin && formData.password.length < 6) {
  ...
  const handleSendResetLink = async () => {
    ...
    try {
      ...
      setResetSuccessMessage(
        `Password reset link sent to ${formData.email}. Check your inbox to set a new password.`
      );
      setView("auth");
      setErrors({});
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const isPasswordValid = isLogin ? true : validatePassword();
  ...
  const getInputBorderClass = (field) => {
    ...
    if (field === "email" && formData.email && !errors.email)
      return "border-green-500";
    if (field === "password" && formData.password && !errors.password)
      return "border-green-500";
    return "border-gray-300";
  };
              ...
              {view === "forgot" ? (
                ...
                >
                  <p className="text-gray-600 text-sm mb-4">
                    Enter your email that you used to create your account to
                  ...
                  <div>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`block w-full pl-10 pr-3 py-3 border ${getInputBorderClass(
                          "email"
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`block w-full pl-10 pr-3 py-3 border ${getInputBorderColor(
                              "name"
                            )} rounded-lg bg-gray极-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${
                              isLoading ? "opacity-70 cursor-not-allowed" : ""
                            }`}
